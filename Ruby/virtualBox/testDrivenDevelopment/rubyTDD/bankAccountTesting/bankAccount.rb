class BankAccount
    ...
    def numOfBankAccounts
        if @@numberOfAccounts == 1
            # p "The bank has #{@@numberOfAccounts} account."
        elsif @@numberOfAccounts > 1
            # p "The bank has #{@@numberOfAccounts} accounts."
            raise ArgumentError.new "You cannot view total number of bank accounts."
            # fail ArgumentError, 'You cannot view total number of bank accounts.'
        end
        return @@numberOfAccounts #does the raise argument error go after this?
    end

    def bankInterestRate
        @@interestRate 
        raise ArgumentError.new "You cannot change the bank interest rate."
    end
